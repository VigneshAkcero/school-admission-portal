...
router.get("/applicants", async (req, res) => {
  ...
  if (parsed.data.search) {
    ...
    where.push(`(a.student_name ILIKE $${values.length} OR a.parent_name ILIKE $${values.length} OR a.mobile_number ILIKE $${values.length} OR COALESCE(a.test_code, '') ILIKE $${values.length})`);
  }
...
router.post("/applicants", async (req, res) => {
  ...
  const result = await pool.query(
    `INSERT INTO applicants (
     ...
     RETURNING id, student_name, parent_name, mobile_number, grade, status, applied_at`,
    [studentName, parentName, mobile, grade, req.user.sub],
  );
...
router.post("/applicants/:id/generate-code", async (req, res) => {
  ...
  try {
    ...
    if (applicant.status !== "pending") {
      await client.query("ROLLBACK");
      return res.status(409).json({ error: "Test code can only be generated for pending applicants" });
    }
