...
export default function PrincipalPage() {
  ...
  const [trends, setTrends] = useState<Array<{ label: string; total_count: number; approved_count: number; rejected_count: number }>>([]);
  ...
  async function loadTrends(nextRange = range) {
    if (!token) return;
    const data = await apiRequest<{ trendData: Array<{ label: string; total_count: number; approved_count: number; rejected_count: number }> }>(
      `/api/principal/trends?range=${nextRange}`,
      { token },
    );
    setTrends(Array.isArray(data.trendData) ? data.trendData : []);
  }
        ...
        <div className="premium-card">
          <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-slate-500">Approved students over time.</p>
            </div>
          ...
          <div className="h-[300px] w-full pr-2 md:h-[320px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trends} margin={{ top: 10, right: 12, left: -20, bottom: 0 }}>
                <defs>
                  <linearGradient id="principalApprovedFill" x1="0" y1="0" x2="0" y2="1">
                ...
                <Area
                  ...
                  name="Approved"
                  stroke="#8b5cf6"
                  fill="url(#principalApprovedFill)"
                  strokeWidth={3}
