...
const SUBJECT_PROGRESS_META = [
  { key: "mathematics", label: "Maths", answeredKey: "mathematics_answered", totalKey: "mathematics_total", tone: "bg-blue-500" },
  { key: "evs", label: "EVS", answeredKey: "evs_answered", totalKey: "evs_total", tone: "bg-emerald-500" },
  { key: "science", label: "Science", answeredKey: "science_answered", totalKey: "science_total", tone: "bg-violet-500" },
  { key: "english", label: "English", answeredKey: "english_answered", totalKey: "english_total", tone: "bg-amber-500" },
  { key: "telugu", label: "Telugu", answeredKey: "telugu_answered", totalKey: "telugu_total", tone: "bg-fuchsia-500" },
  { key: "hindi", label: "Hindi", answeredKey: "hindi_answered", totalKey: "hindi_total", tone: "bg-rose-500" },
] as const;

function getSubjectProgressRows(session: MonitorSession) {
  return SUBJECT_PROGRESS_META.map((subject) => ({
    label: subject.label,
    answered: Number(session[subject.answeredKey] || 0),
    total: Number(session[subject.totalKey] || 0),
    tone: subject.tone,
...
export default function SessionMonitorPage() {
  ...
  const sessionsByCodeRef = useRef<Record<string, SessionWithExtras>>({});

  useEffect(() => {
    sessionsByCodeRef.current = sessionsByCode;
  }, [sessionsByCode]);

  const requestScreensFromStudents = () => {
    if (wsRef.current?.readyState !== WebSocket.OPEN) return;
    Object.values(sessionsByCodeRef.current).forEach((session) => {
                ...
                <div className="lg:grid lg:grid-cols-[minmax(340px,540px)_minmax(0,420px)] lg:items-start">
                  ...
                  <div className="space-y-4 px-5 py-4 lg:max-w-[420px] lg:px-6 lg:py-6">
                    ...
                    <div className="space-y-3">
                      {getSubjectProgressRows(session).map((subject) => (
                        <SubjectProgress
                          key={subject.label}
                          label={subject.label}
          ...
          >
            ...
            <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_320px]">
              ...
              <div className="space-y-5 overflow-y-auto border-l border-slate-200 bg-white p-6">
                ...
                <div className="space-y-3">
                  {getSubjectProgressRows(selectedSession).map((subject) => (
                    <SubjectProgress
                      key={subject.label}
                      label={subject.label}
                      answered={subject.answered}
