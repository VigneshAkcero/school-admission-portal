...
async function migrate() {
  ...
  await pool.query(`
    DO $$ BEGIN
      ALTER TYPE applicant_status ADD VALUE IF NOT EXISTS 'payment_pending';
      ALTER TYPE applicant_status ADD VALUE IF NOT EXISTS 'paid';
    EXCEPTION
      WHEN duplicate_object THEN null;
    END $$;
  `);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS applicants (
      ...
      test_code VARCHAR(12) UNIQUE NULL,
      source VARCHAR(20) NOT NULL DEFAULT 'receptionist',
      email VARCHAR(255) NULL,
      date_of_birth DATE NULL,
      preferred_test_date DATE NULL,
      preferred_test_slot VARCHAR(20) NULL,
      ...
      registration_code VARCHAR(14) UNIQUE NULL,
  ...
  await pool.query(`
    ...
    ALTER COLUMN test_code TYPE VARCHAR(12);
  `);

  await pool.query(`
    ALTER TABLE applicants
    ADD COLUMN IF NOT EXISTS source VARCHAR(20) DEFAULT 'receptionist';
  `);
  ...
  await pool.query(`
    ...
    CHECK (source IN ('receptionist', 'parent_portal'));
  `);
  ...
  await pool.query(`
    ...
    ADD COLUMN IF NOT EXISTS preferred_test_date DATE,
    ADD COLUMN IF NOT EXISTS preferred_test_slot VARCHAR(20);
  `);
  ...
  await pool.query(`
    ALTER TABLE applicants
    DROP CONSTRAINT IF EXISTS applicants_payment_status_check;
  `);

  await pool.query(`
    ALTER TABLE applicants
    ADD CONSTRAINT applicants_payment_status_check
    CHECK (payment_status IN ('not_required','pending','paid','failed'));
  `);

  await pool.query(`
    ALTER TABLE applicants
    ADD COLUMN IF NOT EXISTS registration_code VARCHAR(14) UNIQUE;
  `);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS reg_code_counter (
  ...
  await pool.query(`
    INSERT INTO reg_code_counter (id, last_value)
    VALUES (1, 0)
  ...
  await pool.query(`
    ...
    VALUES ('registration_fee_paise', '50000')
    ON CONFLICT (key) DO NOTHING;
  `);

  await pool.query(`
    INSERT INTO settings (key, value)
    VALUES ('school_code', 'MPS')
    ON CONFLICT (key) DO NOTHING;
  `);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS test_sessions (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      test_code VARCHAR(12) UNIQUE NOT NULL,
      student_name VARCHAR(255) NOT NULL,
      parent_name VARCHAR(255) NOT NULL,
      mobile_number VARCHAR(32) NOT NULL,
      grade INTEGER NOT NULL CHECK (grade BETWEEN 1 AND 9),
  ...
  await pool.query(`
    ALTER TABLE applicants
    DROP CONSTRAINT IF EXISTS applicants_review_status_check;
  `);

  await pool.query(`
    ALTER TABLE applicants
    ADD CONSTRAINT applicants_review_status_check
  ...
  await pool.query(`
    UPDATE applicants
    SET
      score_telugu = ROUND(score * 0.12)::INTEGER,
      score_hindi = ROUND(score * 0.15)::INTEGER
    WHERE status = 'test_completed'
      AND score_telugu IS NULL
