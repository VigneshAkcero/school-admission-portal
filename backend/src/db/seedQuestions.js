...
async function seedQuestions() {
  const questionFileCandidates = [
    path.resolve(process.cwd(), "question_bank_updated.json"),
    path.resolve(process.cwd(), "../data/question_bank_updated.json"),
    path.resolve(process.cwd(), "question_bank.json"),
  ];
  const questionFile = questionFileCandidates.find((candidate) => fs.existsSync(candidate));
  if (!questionFile) {
    throw new Error(`No question bank found. Checked: ${questionFileCandidates.join(", ")}`);
  }
  ...
  if (!questions) {
    throw new Error(`${path.basename(questionFile)} must be an array or contain a top-level 'questions' array`);
  }
