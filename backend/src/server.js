...
app.use("/api/parent", adminCors, parentPortalRoutes);
app.use("/api/applications", adminCors, authenticateAdmin, authorizeRoles("receptionist", "admin"), applicationsRoutes);
...
async function start() {
  ...
  let tlsOptions = null;
  if (config.https) {
    if (!fs.existsSync(config.httpsKeyPath) || !fs.existsSync(config.httpsCertPath)) {
      throw new Error(
        `HTTPS is enabled but certificate files are missing. Expected key=${config.httpsKeyPath}, cert=${config.httpsCertPath}. Run ./scripts/generate-dev-cert.sh`,
      );
    }
  ...
  const server = config.https ? https.createServer(tlsOptions, app) : http.createServer(app);
