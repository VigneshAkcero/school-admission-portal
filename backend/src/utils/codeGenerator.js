...
function normalizeTestCode(testCode) {
  return String(testCode || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
}

function formatTestCode(raw) {
  const normalized = normalizeTestCode(raw);
  if (!/^MPS\d{9}$/.test(normalized)) return raw;
  return `${normalized.slice(0, 3)}-${normalized.slice(3, 5)}-${normalized.slice(5, 7)}-${normalized.slice(7, 9)}-${normalized.slice(9)}`;
}

function formatRegCode(raw) {
  const normalized = String(raw || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
  if (!/^MPS\d{11}$/.test(normalized)) return raw;
  return `${normalized.slice(0, 3)}-${normalized.slice(3, 7)}-${normalized.slice(7, 9)}-${normalized.slice(9, 11)}-${normalized.slice(11)}`;
}
...
async function generateTestCode(gradeOrClient, maybeGrade) {
  ...
  for (let attempt = 0; attempt < 12; attempt += 1) {
    ...
    const testCode = `MPS${yy}${mm}${gg}${xxx}`;
